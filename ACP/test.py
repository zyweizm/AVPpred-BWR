...
# 加载数据
data_4mer = np.load('/mnt/raid5/data2/zywei/wcross-attention/8.ACP_main/data/4mer.npz')
x_train_4mer = data_4mer['x_train']
x_test_4mer = data_4mer['x_test']

data_1mer = np.load('/mnt/raid5/data2/zywei/wcross-attention/8.ACP_main/data/1mer.npz')
x_train_1mer = data_1mer['x_train']
x_test_1mer = data_1mer['x_test']

# 加载并转换标签为分类格式
y_train = pd.read_csv('/mnt/raid5/data2/zywei/wcross-attention/8.ACP_main/data/train/y_train.csv').to_numpy()
y_train = to_categorical(y_train, dtype='int')

y_test = pd.read_csv('/mnt/raid5/data2/zywei/wcross-attention/8.ACP_main/data/test/y_test.csv').to_numpy()
y_test = to_categorical(y_test, dtype='int')

# 实例化两个单独的模型
model_1mer = ourmodel_1mer(input_shape=(50, 150))
model_4mer = ourmodel_4mer(input_shape=(47, 150))
